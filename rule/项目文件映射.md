...
└── src/                          # 源代码目录
    ...
    │   │               │   ├── WebSecurityConfig.java     # Web安全配置类(已添加CORS配置)
│   │               │   ├── CorsConfig.java            # CORS跨域配置类
│   │               │   └── WebConfig.java             # Web配置类(WebMvcConfigurer方式)
    │   │               ├── controller/           # 控制器层
    ...
    │   │               ├── dto/                  # 数据传输对象
    │   │               │   ├── UserChangePasswordRequest.java # 用户修改密码请求DTO
...
### 4. 安全配置模块
- **配置类**: `SecurityConfig.java`, `WebSecurityConfig.java`(已添加CORS配置), `CorsConfig.java`, `WebConfig.java`
- **工具类**: `JwtUtil.java`
...
3. **Web安全配置**: 新增WebSecurityConfig.java，提供更细粒度的安全控制，已添加CORS跨域配置
4. **CORS跨域配置**: 新增CorsConfig.java和WebConfig.java，解决跨域访问问题
